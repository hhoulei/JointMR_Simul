...
setwd("F://wsj-doctor//2024-MR-META//genemo一修//simulation addition//anyF")
...
tiquname <- c('nx', 'ny', 'g', 'beta0', "rho_mean", 'tau', "siga1",
              ...
              "datanum", "F_tat", "F_tat_selected_mean","F_tat_selected_min","weak_selected_pct","weakp")
...
make_colnames <- function(nx, ny) {
   methods <- c("dIVW", "IVW_fixed", "IVW_random", "wme")
                  ...
                  ccname <- c('nx','ny','g','beta0',"rho_mean",'tau',"siga1",
                              ...
                              paste0('MR_meta_old_ivw_random_',c('beta','se','pval')),
                              paste0('MR_meta_old_wme_',c('beta','se','pval')),
                              paste0('MRmeta_',c('beta','se','pval')),"F_tat",
                              "F_tat_selected_mean","F_tat_selected_min","weak_selected_pct","weakp")
}

read_one_result <- function(nx, ny, g, beta0, rho_1, tau, siga1, siga2) {
  ccname <- make_colnames(nx, ny)
  filename <- paste0("Sum_nx_", nx, "_ny_", ny, "_g_", g,'_bx_',bx,
                     "_beta0_", beta0, "_rho1_", rho_1,
                     "_tau_", tau, "_siga1_", siga1,
                     "_siga2_", siga2, "_weakp_0_F_THRESHOLD_10_P_THRESHOLD_5e-08_1128_anyF.csv")
...
p_box <- ggplot(data = ress, aes(x = rho_f, y = beta)) +
  ...
  theme_bw() +
  scale_y_continuous(limits = c(min(beta0b - 0.3), max(beta0b + 0.3)),
                     breaks = box_y_breaks,
...
ggsave(paste0("anyF_fixed_3-5_Estimation_boxplot_all_effects_bx_",bx,".pdf"),
...
for (m in 1:1) {
  ...
  pdf(paste0("anyF_fixed_3-5_qq_bx_",bx,".pdf"), width = 30, height = 7)
  print(final_plot)
  dev.off()
}
...
pdf(paste0("anyF_fixed_3-5_power_all_effects_bx_",bx,".pdf"), width = 30, height = 16)
