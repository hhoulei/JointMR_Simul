...
DataGeneration <- function(g,nx,ny,bx,beta0,tau,rho_1,rho_2,siga1,siga2,weakp){
  ...
  #####exposure
  # Generate one common set of valid SNP IDs and pass all of them to every method.
  # `weakp` is retained for legacy output compatibility, but this corrected
  # scenario no longer creates weak instruments or applies method-specific filters.
  instrument_base <- rnorm(g, bx, 0.03)
  
  seXG <- list()
  betaXG <- list()
  for (i in 1:nx) {
    betaXG[[i]] <- instrument_base + rnorm(g, 0, 0.001)
    target_R2 <- runif(g, 0.09, 0.12)
    target_F <- target_R2 * (g - 1) / (1 - target_R2)
    seXG[[i]] <- abs(betaXG[[i]]) / sqrt(target_F)
  }
...
Comp <- function(i,g,nx,ny,bx,beta0,tau,rho_1,rho_2,siga1,siga2,
  ...
  # All generated SNPs are treated as valid and passed to every method.
  alpha_data1 <- data.frame(alpha_data[common_snp_indices, , drop = FALSE])
  theta_data1 <- data.frame(theta_data[common_snp_indices, , drop = FALSE])
  SNP_GWAS_meta <- length(common_snp_indices)
  ...
  for (m in 1:nx){
    ...
    valid_indices_trad <- common_snp_indices
  ...
  ####MR-meta####
  valid_indices_mrmeta <- common_snp_indices
  SNPnew <- length(valid_indices_mrmeta)
  ...
  if (SNPnew < 2) {
    ...
    WR_matrix <- WR_matrix_full[, valid_indices_mrmeta, drop = FALSE]
    seWR_matrix <- seWR_matrix_full[, valid_indices_mrmeta, drop = FALSE]
  ...
  res_all <- c(nx,ny,g,beta0,rho_1,tau,siga1,siga2,
               SNP_GWAS_meta, SNPMR, SNPnew, # all methods use the same generated SNP count
               rere1,rere2,rere3,
  ...
  write_csv(res_all,file =paste0('Sum_nx_',nx,'_ny_',ny,'_g_',g,
                                 ...
                                 "_all_valid_snps_1128.csv"), append = TRUE)
}
