...
Comp <- function(i,g,nx,ny,bx,beta0,tau,rho_1,rho_2,siga1,siga2,
  ...
  ####MR-meta####
  # --- 筛选策略 3: 仅使用对至少一个暴露, F-stat > 10 的SNP ---
  valid_indices_intersect <- which(
    apply(f_stat_matrix, 1, function(row) any(row >= F_THRESHOLD)) &  # F统计量条件
  ...
  F_tat_selected_mean <- mean(f_stat_matrix[valid_indices_intersect, ])
  F_tat_selected_min  <- mean(apply(f_stat_matrix[valid_indices_intersect, ], 1, min))
  weak_selected_pct   <- mean(f_stat_matrix[valid_indices_intersect, ] < 10) * 100
  # --- 汇总所有结果 ---
  res_all <- c(nx,ny,g,beta0,rho_1,tau,siga1,siga2,
               SNP_GWAS_meta, SNPMR, SNPnew, # 这三个现在都是*筛选后*的SNP计数
               rere1,rere2,rere3,
               theta_hat,theta_se,theta_p_value,F_tat,F_tat_selected_mean,
               F_tat_selected_min,
               weak_selected_pct,weakp) 
  ...
  write_csv(res_all,file =paste0('Sum_nx_',nx,'_ny_',ny,'_g_',g,'_bx_',bx,
